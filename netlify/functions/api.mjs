...
export default async request => {
  ...
  if (apiPath === '/auth/register' && request.method === 'POST') {
    ...
    if (!email || !password || !fullName) {
      await updateDb(current =>
        recordRateLimitAttempt(current, 'register', [ip]),
      );
      return json(
  ...
  if (apiPath === '/auth/login' && request.method === 'POST') {
    ...
    if (
      ...
      await updateDb(current =>
        recordRateLimitAttempt(current, 'login', [ip, email]),
      );
    ...
    if (mode === 'flashcards') {
      ...
      return json(
        applyQuestionFilters(
          sanitizeQuestions(questions, mode),
          url.searchParams,
        ),
      );
    }
  ...
  if (apiPath === '/question-attempts' && request.method === 'GET') {
    ...
    const db = await readDb();
    return json(
      db.attempts.filter(attempt => attempt.user_id === auth.user.id),
    );
  }
  ...
  if (apiPath === '/mock-exams' && request.method === 'POST') {
    ...
    const questions = questionIds
      .map(questionId =>
        evaluateQuestionAnswer(questionId, answers[questionId]),
      )
      .filter(Boolean);

    if (questions.length === 0) {
      return json(
        { message: 'Mock exam questions are required' },
        { status: 400 },
      );
    }
  ...
  if (apiPath === '/profile/reset-progress' && request.method === 'POST') {
    ...
    await updateDb(current => {
      ...
      return {
        ...
        attempts: current.attempts.filter(
          attempt => attempt.user_id !== auth.user.id,
        ),
        mockExams: current.mockExams.filter(
          exam => exam.user_id !== auth.user.id,
        ),
    ...
    const db = await readDb();
    const nextUser =
      db.users.find(user => user.id === auth.user.id) || auth.user;
      ...
      await updateDb(current =>
        syncConfirmedCheckout(
          current,
          {
            ...
            created: Math.floor(
              new Date(checkout.completed_at).getTime() / 1000,
            ),
  ...
  const memberPaymentsMatch = apiPath.match(
  ...
  if (apiPath === '/ai-tutor/conversations' && request.method === 'POST') {
    ...
    await updateDb(current => {
      const currentConversations =
        current.tutorConversations[auth.user.id] || [];
      return {
        ...current,
        tutorConversations: {
  ...
  const tutorStreamMatch = apiPath.match(
    /^\/ai-tutor\/conversations\/([^/]+)\/messages\/stream$/,
  );
  if (tutorStreamMatch && request.method === 'POST') {
    ...
    const currentConversation = (
      db.tutorConversations[auth.user.id] || []
  ...
  const tutorMatch = apiPath.match(
