...
interface DashboardProgress {
  ...
  domain_mastery?: Partial<Record<DomainKey, number>>;
}
...
interface ExamSummary {
  ...
  domain_scores?: Partial<Record<DomainKey, number>> | null;
}
...
function buildWeeklyData(attempts: Attempt[] = []): WeeklyDay[] {
  const today = new Date();
  const days = [] as Array<{ key: string; label: string; total: number; correct: number }>;
  ...
  for (const attempt of attempts) {
    const dateKey = (attempt.created_at ?? '').slice(0, 10);
    const slot = days.find((day) => day.key === dateKey);
    if (slot) {
      slot.total += 1;
      if (attempt.is_correct) slot.correct += 1;
    }
  }
...
export default function AnalyticsScreen() {
  ...
  const fetchData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (dashRes.ok) {
        const nextDashboard = (await dashRes.json()) as DashboardData;
  ...
  const hasActivity = (weekly ?? []).some((day) => day.questions > 0);
  const activeWeekly = (weekly ?? []).filter((day) => day.questions > 0);
  const weeklyAverage = activeWeekly.length
    ? Math.round(activeWeekly.reduce((sum, day) => sum + day.accuracy, 0) / activeWeekly.length)
    : 0;
            ...
            <>
              <View style={s.chart}>
                {weekly.map((day, i) => {
                  ...
                  return (
                    <View key={`${day.label}-${i}`} style={s.chartCol}>
                      ...
                      {day.questions > 0 && <Text style={s.chartQCount}>{day.questions}p</Text>}
                    </View>
                  );
                })}
              </View>
        ...
        <View style={s.panel}>
          {exams === null ? (
            ...
            exams.slice().reverse().map((exam, i) => {
              ...
              return (
                <View key={`${exam.created_at ?? 'exam'}-${i}`} style={[s.examRow, i < exams.length - 1 && s.examRowBorder]}>
                    ...
                    {exam.domain_scores && (
                      <View style={s.examDomains}>
                        {Object.entries(exam.domain_scores).slice(0, 3).map(([key, val]) => {
                          const cfg = DOMAIN_CONFIG.find((domain) => domain.key === key);
                          if (!cfg) return null;
                          const value = typeof val === 'number' ? Math.round(val) : 0;
                          return (
                            <Text key={key} style={s.examDomainChip}>
                              {cfg.label.split(' ')[0]}: {value}%
                            </Text>
                          );
                        })}
                      </View>
