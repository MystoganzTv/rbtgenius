...
GoogleSignin.configure({
  webClientId: '37632251231-4l1t8mg15isp9ck5uvcpelrv5uvj2o66.apps.googleusercontent.com',
  iosClientId: '37632251231-th4qu526qnm34f3uitq7m363dolsu1f0.apps.googleusercontent.com',
  offlineAccess: false,
});
...
export function AuthProvider({ children }) {
  ...
  const loginWithGoogle = async () => {
    ...
    if (__DEV__) console.log('[Google] signIn response:', JSON.stringify(response));

    if (response.type === 'cancelled') return;

    const idToken = response.data?.idToken;

    if (__DEV__) console.log('[Google] idToken:', idToken ? `${idToken.slice(0, 30)}...` : 'NULL');
  ...
  const logout = async () => {
    try {
      ...
      // Sign out of Google silently so next login shows account picker
  ...
  return (
    <AuthContext.Provider value={{ user, token, loading, login, register, loginWithGoogle, logout, refreshDashboard }}>
      {children}
